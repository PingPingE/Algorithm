'''
문제)
기차는 맨 앞에 있는 기관차 1대가 손님이 탄 객차 여러 칸을 끌고 간다. 기관차가 고장나면 기차를 운행할 수 없게 되므로 최근 철도청은 기관차 고장에 대비하여 몇몇 역에 소형 기관차 3대를 배치하기로 결정하였다. 
...
import sys
total=int(input())
li=[0 for _ in range(total+1)]
#===prefix sum
for e,n in enumerate(map(int, sys.stdin.readline().split())):
    li[e+1]=li[e]+n

max_t = int(input())
mem = [[0]*(total+1) for _ in range(4)]

for i in range(1,4): #===i-1번째 기관차에 이어서 i번째 기관차가
    for j in range(max_t,total+1): #===(j-max_t)~j번째 객차를 끌 때의 최대 누적 승객 수는? -> mem[i][j]에 저장
        mem[i][j] = max(mem[i][j-1], li[j]-li[j-max_t]+mem[i-1][j-max_t])
print(mem[-1][-1])
