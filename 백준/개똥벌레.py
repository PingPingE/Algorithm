...
위의 그림에서 4번째 구간으로 개똥벌레가 날아간다면 파괴해야하는 장애물의 수는 총 여덟개이다. (4번째 구간
은 길이가 3인 석순과 길이가 4인 석순의 중간지점을 말한다)
...
bottom_N , top_N = len(info['bottom']), len(info['top'])
bottom_counter = Counter(info['bottom'])
top_counter = Counter(info['top'])

ans = [N+1 for _ in range(H+1)]
for h in range(1,H+1): #종유석 >= h, 석순 >= H-h 개수 카운트
    ...
    for value in range(H-h, H+1):
        try:
            b_count += bottom_counter[value]
        except:
            continue
    print(b_count, t_count)
    ans[h] = b_count + t_count

print(ans)

print(top_counter, bottom_counter)