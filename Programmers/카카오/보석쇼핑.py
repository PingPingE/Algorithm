...
#최적화: 굳이 파라메트릭 서치로 각 윈도우 사이즈(m)마다 전구간을 탐색할 필요가 없었음
def solution(gems):
    set_gems = set(gems)
    N = len(gems)
    kind_N = len(set_gems)
    ...
    #그냥 조건 만족할때까지 계속 윈도우 사이즈 키우다가(e++) 조건 만족하면 줄이면됨
    while s <= e:
        if len(cnt) == kind_N:
...
테스트 13 〉	통과 (1.19ms, 10.4MB)
테스트 14 〉	통과 (0.96ms, 10.4MB)
테스트 15 〉	통과 (2.51ms, 10.5MB)
효율성  테스트
테스트 1 〉	통과 (3.48ms, 10.5MB)
테스트 2 〉	통과 (4.34ms, 10.6MB)
테스트 3 〉	통과 (10.00ms, 11.2MB)
테스트 4 〉	통과 (6.64ms, 11.9MB)
테스트 5 〉	통과 (16.57ms, 11.9MB)
테스트 6 〉	통과 (18.90ms, 12.1MB)
테스트 7 〉	통과 (23.52ms, 12.8MB)
테스트 8 〉	통과 (26.03ms, 12.9MB)
테스트 9 〉	통과 (27.30ms, 13.4MB)
테스트 10 〉	통과 (30.39ms, 13.6MB)
테스트 11 〉	통과 (38.30ms, 14.6MB)
테스트 12 〉	통과 (26.46ms, 15.9MB)
테스트 13 〉	통과 (36.60ms, 16.6MB)
테스트 14 〉	통과 (60.41ms, 17.1MB)
테스트 15 〉	통과 (62.69ms, 17.9MB)
'''
