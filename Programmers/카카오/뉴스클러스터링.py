...
이를 이용하여 문자열 사이의 유사도를 계산하는데 이용할 수 있다. 문자열 FRANCE와 FRENCH가 주어졌을 때, 이를 두 글자씩 끊어서 다중집합을 만들 수 있다. 각각 {FR, RA, AN, NC, CE},
{FR, RE, EN, NC, CH}가 되며, 교집합은 {FR, NC}, 합집합은 {FR, RA, AN, NC, CE, RE, EN, CH}가 되므로, 두 문자열 사이의 자카드 유사도 J("FRANCE", "FRENCH") = 2/8 = 0.25가 된다.
...
'''
#정규표현식으로 문자열을 검사하고 각각의 원소들을 카운트한다 -> A,B
#A,B 각각의 key값으로 합집합, 교집합을 구하고 value값으로 중복도를 구한다.
...
def solution(str1, str2):
    ...
    for i in range(len(str1)-1):
        if re.fullmatch('[a-z]*',str1[i:i+2])is None: continue
        A[str1[i:i+2]] += 1
    for i in range(len(str2)-1):
        if re.fullmatch('[a-z]*',str2[i:i+2])is None: continue
        B[str2[i:i+2]] += 1
    set_A = set(A.keys())
    set_B = set(B.keys())
    AorB = set_A|set_B
    ...
    for j in AandB:
        cnt2+=min(A[j],B[j])
    return 65536 if cnt1==0 else int(65536*cnt2/cnt1)

'''
정확성  테스트
테스트 1 〉	통과 (0.13ms, 9.98MB)
테스트 2 〉	통과 (0.15ms, 9.95MB)
테스트 3 〉	통과 (0.13ms, 9.75MB)
테스트 4 〉	통과 (2.30ms, 9.96MB)
테스트 5 〉	통과 (0.17ms, 9.98MB)
테스트 6 〉	통과 (0.12ms, 9.95MB)
테스트 7 〉	통과 (0.32ms, 10.1MB)
테스트 8 〉	통과 (0.14ms, 9.86MB)
테스트 9 〉	통과 (0.30ms, 10MB)
테스트 10 〉	통과 (0.48ms, 10.1MB)
테스트 11 〉	통과 (0.72ms, 10.1MB)
테스트 12 〉	통과 (0.11ms, 9.86MB)
테스트 13 〉	통과 (0.26ms, 9.68MB)
'''