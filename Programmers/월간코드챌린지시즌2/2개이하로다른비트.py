...
def solution(numbers):
    ...
    for n in numbers:
        tmp=list(bin(n))
        cnt=0
        #뒤에서 부터 보면서 0인 부분이 있으면 1로 바꾸고 바로 뒤에 있는 것도 0으로 바꿔주어야 '제일 작은 수'가 된다.
        for e,s in enumerate(bin(n)[:1:-1],1):
            if s=='0':
                tmp[-e]='1'
                
                #여기서 한 칸 뒤에 있는 것을 0으로 바꿔줘야 '제일 작은 수'가 됨
                if e>1:
                    tmp[-(e-1)]='0'
...
'''
정확성  테스트
테스트 1 〉	통과 (1.48ms, 10.3MB)
테스트 2 〉	통과 (112.41ms, 26.1MB)
테스트 3 〉	통과 (0.16ms, 10.2MB)
테스트 4 〉	통과 (1.29ms, 10.3MB)
테스트 5 〉	통과 (1.54ms, 10.3MB)
테스트 6 〉	통과 (1.35ms, 10.3MB)
테스트 7 〉	통과 (130.42ms, 25.4MB)
테스트 8 〉	통과 (133.78ms, 24.9MB)
테스트 9 〉	통과 (128.58ms, 24.2MB)
테스트 10 〉	통과 (293.73ms, 26.8MB)
테스트 11 〉	통과 (291.44ms, 26.8MB)
채점 결과
정확성: 100.0
합계: 100.0 / 100.0
'''
