...
def solution(enroll, referral, seller, amount):
    ...
    def func(x, amount):
        ...
        if x == '-' or amount == 0:
            return
        # 상위 노드에 배분할 값
        give = math.floor(amount * 0.1)
        ans[x] += amount - give
        return func(link[x], give)

    for se, am in zip(seller, amount):
        origin = am * 100
        #상위 노드에 배분할 값
        give = math.floor(origin * 0.1)
        ans[se] += origin - give
        func(link[se], give)
...
테스트 6 〉	통과 (1.76ms, 13MB)
테스트 7 〉	통과 (1.88ms, 13.2MB)
테스트 8 〉	통과 (3.80ms, 13.2MB)
테스트 9 〉	통과 (19.19ms, 14.2MB)
테스트 10 〉	통과 (167.76ms, 21.3MB)
테스트 11 〉	통과 (151.13ms, 20.9MB)
테스트 12 〉	통과 (150.13ms, 20.8MB)
테스트 13 〉	통과 (139.98ms, 20.8MB)
'''